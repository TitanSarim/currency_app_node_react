...
const Record = () => {
    ...
    const handleMultipleClick = async () => {
        ...
        try {
            ...
            const response = await axios.delete(`http://localhost:3800/api/v1/deleteCurrencies/${item.userid}`);
            console.log(`Deleted item with userid ${item.userid}:`, response.data);
        }
            window.location.reload()
        } catch (error) {
            setError(error.message || 'An error occured')
                ...
                <tbody>
                        {data?.map((item) => (
                            <tr  key={item.userid}>
                                ...
                                <button onClick={() => handleSingleClick(item?.userid)}>
                                    <img src={bin} width={20} alt='bin'/>
                                </button>
                                </td>
                            </tr>
